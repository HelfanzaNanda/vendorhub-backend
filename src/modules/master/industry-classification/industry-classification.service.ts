...
export class IndustryClassificationService {
    ...
    async create(data: CreateIndustryClassificationDto) {
        return this.industryClassificationRepo.save(
            this.industryClassificationRepo.create(data),
        );
    }
    ...
    async pagination(query: PaginationQueryDto) {
        ...
        const selectColumns = Object.values(INDUSTRY_CLASSIFICATION_FIELDS).map(
            (f) => f.column,
        );
    ...
    async findOne(id: number) {
        const industryClassification =
            await this.industryClassificationRepo.findOne({
                select: {
                    createdByUser: {
                        username: true,
                    },
    ...
    async update(id: number, data: UpdateIndustryClassificationDto) {
        const industryClassification =
            await this.industryClassificationRepo.findOne({ where: { id } });
