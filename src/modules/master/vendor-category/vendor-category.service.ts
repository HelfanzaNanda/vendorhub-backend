...
export class VendorCategoryService {
    ...
    async create(data : CreateVendorCategoryDto) {
        return this.vendorCategoryRepo.save(this.vendorCategoryRepo.create(data));
    }
    ...
    async pagination(query: PaginationQueryDto) {
        ...
        const selectColumns = Object.values(VENDOR_CATEGORY_FIELDS).map(f => f.column);
    ...
    async update(id: number, data : UpdateVendorCategoryDto) {
        const vendorCategory = await this.vendorCategoryRepo.findOne({ where: { id } });
        if (!vendorCategory) throw new NotFoundException(`Data with id ${id} not found`);
